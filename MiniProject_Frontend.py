	...
	def __init__(self, root):
		...
		def adddata():
			if(len(Movie_ID.get())!=0):
				app.AddMovieRec(Movie_ID.get(),Movie_Name.get(),Release_Date.get(),Director.get(),Cast.get(),Budget.get(),Duration.get(),Rating.get())
		...
		def deldata():
			if(len(Movie_ID.get())!=0):
				app.DeleteMovieRec(sd[0])
				clcdata()
				disdata()

		def searchdb():
			MovieList.delete(0,END)
			for row in app.SearchMovieData(Movie_ID.get(),Movie_Name.get(),Release_Date.get(),Director.get(),Cast.get(),Budget.get(),Duration.get(),Rating.get()):
				MovieList.insert(END, row, str(""))
